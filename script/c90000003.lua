...
function s.efftg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	-- Edison/pre-errata:
	-- Option 1 should only be available if this card is still face-up on the field.
	-- If Lord British is destroyed in battle together with the opponent's monster,
	-- only options 2 and 3 should be available.
...
function s.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	if op==1 then
		--This card can make another attack in a row.
		--Do not use CanChainAttack() in the target check, because in Edison
		--this option can be chosen again after each battle destruction.
		local c=e:GetHandler()
